...
class SphereSnake(object):
    def __init__(self, M_1, M_2, nSamplesPerSeg=11, hidePoints=True):
        ...
        self.freeParams = [Point3D(0, 0, 0) for _ in range(self.nFreeParams)]
        ...
        # Parameter for hiding control points
    ...
    def _updateCoefs(self):
       ...
       for l in range(1, M_2):
           for k in range(M_1):
               self.coefs[k + (l+1)*M_1].updateFromPoint(self.freeParams[k + (l-1)*M_1])
       ...
       for k in range(M_1):
           ...
           # c[k, -1]
           self.coefs[k].updateFromPoint(self.coefs[k + 2*M_1] + (T_1N*CM1 + T_2N*SM1)/(M_2*phi_2_prime_1))
           
           # c[k, M_2+1]
           self.coefs[k + (M_2+2)*M_1].updateFromPoint(self.coefs[k + M_2*M_1] - (T_1S*CM1 + T_2S*SM1)/(M_2*phi_2_prime_1))

           # c[k, 0]
           self.coefs[k + M_1].updateFromPoint(CN/phi_2_0 - (self.coefs[k] + self.coefs[k + 2*M_1])*phi_2_1/phi_2_0)
            
           # c[k, M_2]
           self.coefs[k + (M_2+1)*M_1].updateFromPoint(CS/phi_2_0 - (self.coefs[k + M_2*M_1] + self.coefs[k +
                                                                                           (M_2+2)*M_1])*phi_2_1/phi_2_0)
    ...
    def _getScale(self, i):
        ...
        if i >= 0 and i < self.MR_1:
            ...
            scale = Snake3DScale(points=points, closed=False, color=Color(0, 0, 204))
    ...
    def getNodes(self):
        nodes = self.coefs[:self.M_1*(self.M_2+3)]
        return nodes
